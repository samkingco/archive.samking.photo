...
function _getMeta(image, callback) {
    ...
    gm(image.path).identify(identifyString, function (err, value) {
        if (!err) {
...
function _formatMeta(image, metaArr) {
    ...
    // Format and set dimensions
    const dimensions = metaArr[7].split('x');
    image.dimensions = {
        width: dimensions[0].toString(),
        height: dimensions[1].toString()
    };
...
const imageProcessComposer = async.compose(_getModifiers, _getMeta);
