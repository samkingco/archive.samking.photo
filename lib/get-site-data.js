...
const cfOptions = {keypairId: secret.cloudfrontAccessKey, privateKeyString: secret.cloudfrontSecretKey};
...
function _transformInputImages(imageList) {
    _.each(imageList, function (image) {
        _.each(image.sizes, function (size, key) {
            if (config.useCDN) {
                var cfUrl = config.cdnURL+'/'+size;
                console.log(cfUrl);
                // var signedUrl = cf.getSignedUrl(cfUrl, cfOptions);
                image.sizes[key] = cfUrl;
            } else {
